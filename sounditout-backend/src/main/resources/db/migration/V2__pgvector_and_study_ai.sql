...
DO $$
    BEGIN
        -- FK: report_embedding.student_id -> students.id / student.id
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='students') THEN
            EXECUTE 'ALTER TABLE report_embedding
             ADD CONSTRAINT fk_embedding_student
             FOREIGN KEY (student_id) REFERENCES students(id) ON DELETE CASCADE';
        ELSIF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='student') THEN
            EXECUTE 'ALTER TABLE report_embedding
             ADD CONSTRAINT fk_embedding_student
        ...
        -- FK: report_embedding.report_id -> progress_reports.id / progress_report.id
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='progress_reports') THEN
            EXECUTE 'ALTER TABLE report_embedding
             ADD CONSTRAINT fk_embedding_report
             FOREIGN KEY (report_id) REFERENCES progress_reports(id) ON DELETE CASCADE';
        ELSIF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='progress_report') THEN
            EXECUTE 'ALTER TABLE report_embedding
             ADD CONSTRAINT fk_embedding_report
        ...
        -- FK: study_plan.student_id -> students.id / student.id
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='students') THEN
